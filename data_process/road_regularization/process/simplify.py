...
def Convert_single(osm_path, threshold):
    ...
    arcpy.PolygonToCenterline_topographic(
        "temp_buffered_dissolved", "road_singleLine_proj"
    )
...
def Check_topo(roads):
    ...
    arcpy.CreateFeatureDataset_management(
        out_name=dataset_name, spatial_reference=Project_ref
    )
    ...
    arcpy.AddFeatureClassToTopology_management(
        f"{dataset_name}/{topology_name}", f"{dataset_name}/{topo_data_name}"
    )
    # print("adding topo rules...")
    # validate 断头点
    arcpy.AddRuleToTopology_management(
        f"{dataset_name}/{topology_name}",
    ...
    arcpy.ExportTopologyErrors_management(
        "{0}/{1}".format(dataset_name, topology_name), out_basename="err"
    )
...
def Clean_spike(roads, pts, threshold, CITY, keep_spike=False):
    ...
    if keep_spike:
        arcpy.FeatureClassToFeatureClass_conversion(
            pts, OUTPUT_PATH, f"err_points_{CITY}.shp"
        )
        arcpy.FeatureClassToFeatureClass_conversion(
            "spike_roads", OUTPUT_PATH, f"err_lines_{CITY}.shp"
        )
    spike = "spike_roads"
    ...
    arcpy.FeatureClassToFeatureClass_conversion(
        spike, out_name="to_be_cut", where_clause=expression
    )
...
def Join_attributes(origin, master):
    ...
    field_map.addInputField(origin, ROAD_TYPE_FIELD)
    out_field = field_map.outputField
    out_field.name = ROAD_TYPE_FIELD
    out_field.aliasName = ROAD_TYPE_FIELD
