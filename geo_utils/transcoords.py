...
MC2LL = [[1.410526172116255e-8, 0.00000898305509648872, -1.9939833816331, 200.9824383106796, -187.2403703815547,
         ...
         [-1.981981304930552e-8, 0.000008983055099779535, 0.03278182852591, 40.31678527705744,
          0.65659298677277, -4.44255534477492, 0.85341911805263, 0.12923347998204, -0.04625736007561, 4482777.06],
         [3.09191371068437e-9, 0.000008983055096812155, 0.00006995724062, 23.10934304144901, -
         0.00023663490511, -0.6321817810242, -0.00663494467273, 0.03430082397953, -0.00466043876332, 2555164.4],
...
def convertor(cC, cD):
    ...
    cB = abs(cC.y) / cD[9]
    cE = cD[2] + cD[3] * cB + cD[4] * cB * cB + cD[5] * cB * cB * cB + cD[6] * cB * \
        cB * cB * cB + cD[7] * cB * cB * cB * cB * \
        cB + cD[8] * cB * cB * cB * cB * cB * cB
    if (cC.x < 0):
        T = T * -1
    else:
        T = T
...
def bdmc2wgs84(x, y):
    x = float(x)
    y = float(y)
    bd09_xy = mercatortobd09(x, y)
    gcj02_xy = bd09togcj02(*bd09_xy)
    wgs84_xy = gcj02towgs84(*gcj02_xy)
    return wgs84_xy
...
def bdmc2bdll(X, Y):
    ...
    BDlon = cE[0] + cE[1] * X
    cC = Y / cE[9]
    BDlat = cE[2] + cE[3] * cC + cE[4] * cC * cC + cE[5] * cC * cC * cC + cE[6] * cC * cC * cC * cC + \
...
def bd2gcj(bdLat, bdLng):
    ...
    x = bdLng - 0.0065
    y = bdLat - 0.006
    z = math.hypot(x, y) - 0.00002 * math.sin(y * math.pi * 3000.0 / 180.0)
    theta = math.atan2(y, x) - 0.000003 * \
        math.cos(x * math.pi * 3000.0 / 180.0)
...
# YQ's new function for convert bd mc coords to wgs lat & long
def bdmc2wgs(X, Y):
    BDlon, BDlat = bdmc2bdll(X, Y)
    wgs_lat, wgs_lng = bd2wgs(BDlat, BDlon)
