...
class Validator(object):
    ...
    def _validate_aoi_filter_settings(cls) -> None:
        ...
        cls._verify_value_type(Repo._min_similarity, "min_similarity", float or int)
    ...
    def _verify_non_negative_num(cls, value: any, name: str) -> None:
        cls._verify_value_type(value, name, float or int)
        if value < 0:
            raise ValueError(f'"{name}" must be a non-negative number.')

    @staticmethod
    def _verify_value_type(
        value: any, name: str, type: bool or int or str or list or dict
    ) -> None:
